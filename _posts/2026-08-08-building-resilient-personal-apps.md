...
green. I know which version is running on the box under my desk. [Aotearoa,
...
side if you're curious. This post is mostly about the maintenance side of that.
...
Minor and major bumps wait for me. I've drawn that line deliberately. Patches are
routine; majors might need a closer look.
...
releases and pinned images. But there's another layer I've been pondering: the
small tidy-ups you notice and then don't get around to. A deprecation warning
that's been sitting in the logs for a month. A test that's a bit flaky but not
quite broken. A dependency that's technically current but sitting on an older
major. Low-stakes one at a time, important in aggregate, and tedious to
context-switch into.
...
noticed this and here's a PR" stuff while I'm away, with CI as a
...
I'm optimising for — the rest of the [gardening](/posts/2024/05/27/Passion-projects.html)
can wait until I actually feel like doing it.
